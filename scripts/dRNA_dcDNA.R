...
# CLI Executable R Script
# Author: Oguzhan Begik (modified for CLI by Code Copilot)
# ---------------------------
...
process_func <- function(stats_file_path, method) {
  ...
  stats_file <- stats_file %>%
    filter(pos > 20) %>%
    mutate(
      mismatch = rowSums(select(., A, T, C, G), na.rm = TRUE) - .[[.$ref_nuc[1]]],
...
mismatch_profile_barplot_facet <- function(data, label) {
  # Assign color: gray for ref base, custom for others
  base_colors <- c("A" = "#1fab89", "T" = "#eb4d55", "C" = "#1e56a0", "G" = "#f0cf85")
  data$fill_color <- ifelse(data$variable == data$Group.2, "#888888", base_colors[data$variable])

  pdf(file = file.path(output_dir, paste0(label, "_mismatch_profile_barplot.pdf")), 
      height = 1.5, width = 6)

  print(
    ggplot(data, aes(x = Group.1, y = value, fill = fill_color)) +
